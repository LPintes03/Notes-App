    ...
    <div class="view">
        ...
        <div class="form">
            ...
            <form action="{{ route('updateNote', ['id' => $note->id]) }}" method="POST">
                @csrf
                @method('PUT') <!-- This directive overrides the POST request to use PUT -->

                <div>
                    <input type="text" name="title" class="h1" value="{{ $note->title }}" maxlength='60'>
                    @if ($errors->has('title'))
                    <span class="error">{{ $errors->first('title') }}</span>
                    @endif
                </div>
                <div>
                    <input type="text" name="description" class="description" placeholder="Description..." value="{{ $note->description }}" maxlength='150'>
                    @if ($errors->has('description'))
                    <div class="error">{{ $errors->first('description') }}</div>
                    @endif
                </div>
                <div>
                    <textarea name="content" class="scroll-container" id="content" maxlength='10000'>{{ $note->content }}</textarea>
